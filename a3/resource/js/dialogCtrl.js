export class DialogueSystem {
    ...
    triggerDialogue(logic) {
        ...
        // Trigger meetAria dialogue
        if (logic.player.score >= 10 && !this.isSceneCompleted["meetAria"]) {
            this.isPaused = true;
            this.switchDialog("meetAria");
        }

        // Trigger encounterZephyr dialogue
        if (logic.player.score >= 15 && !this.isSceneCompleted["encounterZephyr"]) {
            this.isPaused = true;
            this.switchDialog("encounterZephyr");
        }

        // Trigger midBattle dialogue
        if (logic.player.score >= 25 && !this.isSceneCompleted["midBattle"]) {
            this.isPaused = true;
            this.switchDialog("midBattle");
        }

        // Trigger finalBattle dialogue
        if (logic.player.score >= 35 && !this.isSceneCompleted["finalBattle"]) {
            this.isPaused = true;
        ...
        if (logic.player.isGameOver) {
            this.isPaused = true;
            this.switchDialog(logic.player.score >= 40 ? "win" : "lose");
        }
    }
